...
X_AXIS_LENGTH=0.08
QSIM_MODEL_PATH = os.path.join(YML_PATH, "leap_3d_planar_rotate.yml")
MODEL_PARAM_PATH = os.path.join(CONFIG_PATH, "leap_3d_planar_rotate.yaml")
...
class QSimDDPParams(object):
    def __init__(self):
        ...
        self.dtrans = -0.05   # translation step size in y (default: 0.03)
        self.target_trans = -np.pi   # set fixed target

        # weight
        self.w_u = [1e1, 1e1, 5e2]                       # default: [1e1, 1e1, 1e2] 
        self.w_u_T = [1e2, 1e2, 5e3]                    # default: [1e2, 1e2, 1e3]

        self.u_lb = -0.2 * np.ones(self.nu)             # -0.12
        self.u_ub = 0.2 * np.ones(self.nu)              # 0.12
        self.execution_scale = 0.5                       # 0.5, positive scalar, scale u0 when execution (i.e., rollout new x0)
...
def solve_ddp_trajopt_problem(
    ...
    np.save("../optimizer/warmstart/xs_trival_planar_rotate_leap.npy", solver.xs)
    np.save("../optimizer/warmstart/us_trival_planar_rotate_leap.npy", solver.us)
...
def generate_reference_from_x0(x0, params:QSimDDPParams):
    ...
    trans0 = x0[params.q_u_indices[2]]
    ...
    xu_ref[:, params.q_u_indices[2]] = trans_ref
...
def prepare_everything_for_ROS():
    ...
    xs_init, us_init = np.load("../optimizer/warmstart/xs_trival_planar_rotate_leap.npy"), \
                        np.load("../optimizer/warmstart/us_trival_planar_rotate_leap.npy")

    dtheta = 1.25 * np.pi/200
    q_ref = np.zeros((100, params.nx))
