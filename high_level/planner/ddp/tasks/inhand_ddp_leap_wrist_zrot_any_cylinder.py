...
X_AXIS_LENGTH=0.08
QSIM_MODEL_PATH = os.path.join(YML_PATH, "leap_3d_wrist_zrot_any_cylinder.yml")
MODEL_PARAM_PATH = os.path.join(CONFIG_PATH, "leap_3d_wrist_zrot_any_cylinder.yaml")
...
class QSimDDPParams(object):
    def __init__(self):
        ...
        self.num_wrist_joints = 1   # full: 6
        self.nx = 17 + self.num_wrist_joints
        self.nu = 16 + self.num_wrist_joints
        # self.q_u_indices = [16]
        self.q_u_indices = [0]
        self.q_a_wrist_indices = [i for i in range(1, 1+self.num_wrist_joints)]
        self.q_a_hand_indices = [i for i in range(1+self.num_wrist_joints, self.nx)]
...
def solve_ddp_trajopt_problem(
    ...
    np.save("../optimizer/warmstart/xs_trival_leap_wrist_cylinder.npy", solver.xs)
    np.save("../optimizer/warmstart/us_trival_leap_wrist_cylinder.npy", solver.us)
...
def solve_ddp_mpc_problem(
    ...
    wrist_poses = np.linspace(-np.pi/6, np.pi/6, N-T)
...
# Utility functions
def make_ddp_params(ddp_params):
    ...
    param.execution_scale = 0.5
    param.dxu = 5 * np.pi / 200
    param.target_xu = np.atleast_1d(3.14159)
    param.use_target_xu = False
    param.u_lb = -0.12 * np.ones(param.nu)
    param.u_ub = 0.12 * np.ones(param.nu)
...
def set_init_and_reg_states(param:DDPSolverParams, q_sim, extras):
    ...
    # set state limits
    j_lb, j_ub = load_joint_limits(joint_names, xa_reg, extras['jlimits'])
...
def prepare_everything_for_ROS():
    ...
    loaded_ddp_params = make_ddp_params(loaded_params['ddp_params'])
    ...
    loaded_ddp_params.n_wrist = 1
    loaded_ddp_params.q_wrist_indices = [loaded_ddp_params.q_u_indices[-1]+1+i for i in range(loaded_ddp_params.n_wrist)]
    ...
    xs_init, us_init = np.load("../optimizer/warmstart/xs_trival_leap_wrist_cylinder.npy"), \
                        np.load("../optimizer/warmstart/us_trival_leap_wrist_cylinder.npy")
